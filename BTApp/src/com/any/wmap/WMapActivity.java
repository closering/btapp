...
public class WMapActivity extends FragmentActivity implements
...
//	private String TargetURL_MapMarker = "http://192.168.11.241:8080/bt/com/conn/android/locusMark";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		requestWindowFeature(Window.FEATURE_NO_TITLE);//软件没有标题显示
		...
		button_trace.setOnClickListener(new OnClickListener() {  
            ...
            public void onClick(View v) { 
		        		...
		        		try{
		        			...
		        			//System.out.println("Point_info:"+Point_info.substring(Point_info.length()-2000, Point_info.length()));
		        			System.out.println("Point_info:"+Point_info);
		        	    	...
		        	    	if(point_info.length>=5){
		                	    ...
		                	    {	
		                	    	...
		                	    	else
		                	    	{
		                  			 ...
		                  			 dd[0] = dd[2];
		                  			 dd[1] = dd[3];
		                	    }//end  for(int i=5; i < point_info.length; i += 4)
		        	    	}       //end if(point_info.length>=5)
		        	    }//end   for(int j=0; j < point_info_package.length; j++)
		        		...
		        		try{
		        			...
		        			//Toast.makeText(WMapActivity.this, Marker_info, Toast.LENGTH_SHORT).show();
		        		}catch(Exception e){
		        			e.printStackTrace();
		        		}
		        	
		        		System.out.println("Marker_info:"+Marker_info);
		    	    	...
		    	    	if(marker_info.length>=5){
		            	    ...
		            	    {	
		               		...
		               		// System.out.println("44444444");
		               			...
		               			aMap.moveCamera(CameraUpdateFactory.newLatLngZoom(marker, 12));
		            	    }
		    	    	}  */ 

		        		/*marker = new LatLng(39.999431,116.331387); 
		       			mymarker=aMap.addMarker(new MarkerOptions().position(marker).title("清华")
		       					 .snippet("伟伦楼")
		       			         .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE))
		       			         .draggable(false));
		       		// System.out.println("44444444");
		       			...
		       			marker = new LatLng(39.9656,116.3624); 
		       			mymarker=aMap.addMarker(new MarkerOptions().position(marker).title("北邮")
		       					 .snippet("科技大厦")
		       			         .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE))
		       			         .draggable(false));
		       		// System.out.println("44444444");
...
/*
	...
	public void setupMap(){
		UiSettings uiSettings=aMap.getUiSettings();//返回地图的用户界面设置对象。
		uiSettings.setCompassEnabled(true);	//是否显示指南针
		//uiSettings.setMyLocationButtonEnabled(true);
		uiSettings.setScaleControlsEnabled(true);//设置比例尺功能是否可用
		...
		uiSettings.setTiltGesturesEnabled(true);//倾斜旋转以获取3D效果,是否允许倾斜手势
		//指定北京经纬度
		LatLng sh=new LatLng(39.999744,116.330124);
		//CameraUpdateFactory.newLatLng(sh);////回一个移动目的地的屏幕中心点的经纬度的CameraUpdate 对象。移动后的地图缩放级别改变，缩放级别为4（固定值）。
		aMap.moveCamera(CameraUpdateFactory.newLatLngZoom(sh,12));//newLatLngZoom返回一个CameraUpdate 对象，包括可视区域框移动目标点屏幕中心位置的经纬度以及缩放级别。
	...
	*/
	...
	public void onLocationChanged(AMapLocation location) {
		...
		System.out.println("定位成功："+Location_info);
		...
		if(init_flag_start == true)
		{
			if(init_flag == false)
			{
	    		...
	    		Toast.makeText(WMapActivity.this, "Init_State: "+Init_state, Toast.LENGTH_SHORT).show();
	    		init_flag = true;
			}//end if(init_flag == false)
		...
		public void handleMessage(Message msg) {
			if (msg.what == Constants.POISEARCH) {
				...
				if (routeResult != null && routeResult.size() > 0) {
					...
					if (route != null) {
						routeOverlay = new RouteOverlay(WMapActivity.this,
								aMap, route);
			...
			public void run() {
				try {
					PoiSearch poiSearch = new PoiSearch(
							WMapActivity.this, new PoiSearch.Query(
	...
	public void routeSearch(LatLonPoint startPoint, LatLonPoint endPoint){
		progDialog = ProgressDialog.show(WMapActivity.this, null,
				"正在获取线路", true, true);
		...
		Thread t = new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					routeResult = Route.calculateRoute(WMapActivity.this,
							fromAndTo, mode);
	...
	class Map_trace extends Thread{
		...
		public void run(){
			...
			try
			{
				...
				params.add(new BasicNameValuePair("locusDownload", "bid,"+Bid));
                System.out.println("locusDownload:"+Bid);
	...
	class Point_upload extends Thread{
		...
		public void run(){
			...
			try
			{
				...
				String Upload = username + "," + Init_state + "," + Location_info + "," + order + "," + update_time ;
	...
	class User_init extends Thread{
		...
		public void run(){
			...
			try
			{
				...
				List<NameValuePair> params = new ArrayList<NameValuePair>();
				params.add(new BasicNameValuePair("init", username+","+Bid));
	...
	class Map_Marker extends Thread{
		...
		public void run(){
			...
			try
			{
				...
				List<NameValuePair> params = new ArrayList<NameValuePair>();
				//params.add(new BasicNameValuePair("locusMark", "bid,"+Bid));
				params.add(new BasicNameValuePair("locusMark", "bid,"+Bid));
				HttpEntity entity = new UrlEncodedFormEntity(params, HTTP.UTF_8);
				request.setEntity(entity);
				HttpResponse httpResponse = httpClient.execute(request);
